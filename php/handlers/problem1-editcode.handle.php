...
*/
    ...
    $query = "SELECT problem_id, problem_code FROM problems WHERE problem_code = ?";
    ...
    // Query string
    $query = "UPDATE problems SET problem_code = ? WHERE problem_name = ?";

    // A bunch of attempts to do the fckin statement
    if($statement = mysqli_prepare($LINK, $query)){
        mysqli_stmt_bind_param($statement, "ss", $code, $problem);
